...
class SupabaseService:
    ...
    def get_total_lion_population(self, management_unit: Optional[str] = None, year: Optional[int] = None) -> float:
        """
        Calculate total lion population from stored grid-cell observations only.
        """
        ...
        if year:
            ...
            return sum(cell.get("baseline_lion_density", 0) for cell in (result.data or []))
        ...
        return sum(cell.get("baseline_lion_density", 0) for cell in result.data)
    ...
    def get_landscape_stats(self, management_unit: Optional[str] = None, year: Optional[int] = None) -> Dict[str, Any]:
        """
        Comprehensive landscape statistics calculated from stored observations.
        """
        # 1. Total lions for the requested stored year (if any)
        ...
        return {
            ...
            "avg_nightlight": round(base_nightlight, 4),
            "high_risk_cell_count": high_risk_cells,
            "management_unit_count": 1 if management_unit else len(self.get_management_units()),
            "year_adjusted": False
        }
