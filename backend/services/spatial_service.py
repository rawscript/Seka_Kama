...
async def get_baseline_grid(
    ...
    for row in result.data:
        # Never manufacture densities. A missing value is exposed as zero with
        # an explicit completeness flag so the data issue can be corrected.
        baseline_density = row.get('baseline_lion_density')
        lion_density_value = float(baseline_density) if baseline_density is not None else 0.0
        ...
        features.append({
            ...
            "properties": {
                ...
                "lion_density_available": baseline_density is not None,
