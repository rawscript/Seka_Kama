...
async def enrich_cells_with_live_data(
    ...
    If a live source is unavailable, existing stored cell values are preserved.
    """
    if not cells:
        return cells
    ...
    if rainfall_mm is None:
        logger.warning("[EcoEnrich] NASA POWER unavailable — preserving stored rainfall values")

    # ── 3. Fetch prey density (GBIF) ──────────────────────────────────────
    prey_density = await fetch_gbif_prey_density(clon, clat, 50, year)
    if prey_density is None:
        logger.warning("[EcoEnrich] GBIF unavailable — preserving stored prey values")

    logger.info(
        f"[EcoEnrich] Live data availability → Rainfall: {rainfall_mm is not None} | "
        f"Prey density: {prey_density is not None}"
    )
    ...
    for cell in cells:
        ...
        updated = dict(cell)
        if rainfall_mm is not None:
            updated["annual_rainfall_mm"] = rainfall_mm
            updated["hwc_risk_score"] = derive_hwc_risk(nightlight, dist_prot, rainfall_mm)
        if prey_density is not None:
            updated["prey_density"] = prey_density
        # Do not substitute proxy variables or damp model inputs: XGBoost is
        # evaluated only with observed stored features and live measurements.
...
async def get_live_ecosystem_indicators(management_unit: Optional[str] = None, year: Optional[int] = None) -> List[Dict[str, Any]]:
    """
    Return only indicators backed by a live source. Unsupported indicators are
    omitted rather than filled with regional constants or invented trends.
    """
    if year is None:
        year = datetime.now().year - 1
        
    # Mara study-area centroid. Management-unit centroids require persisted
    # geometry metadata and are intentionally not guessed here.
    lon, lat = 35.24, -1.52
...
async def get_live_environmental_conditions(management_unit: Optional[str] = None, year: Optional[int] = None) -> Dict[str, Any]:
    ...
    nasa_data = await fetch_complete_nasa_data(lon, lat, year)
    required = ("T2M", "RH2M", "WS2M", "PRECTOTCORR", "CLOUD_AMOUNT", "GWETROOT")
    missing = [field for field in required if nasa_data.get(field) is None]
    if missing:
        raise RuntimeError(f"NASA POWER did not return required fields: {', '.join(missing)}")
    
    return {
        "temperature": round(nasa_data["T2M"], 1),
        "humidity": round(nasa_data["RH2M"], 1),
        "wind_speed": round(nasa_data["WS2M"], 1),
        "precipitation": round(nasa_data["PRECTOTCORR"] / 365.25, 1),
        "cloud_cover": round(nasa_data["CLOUD_AMOUNT"], 1),
        "uv_index": None,
        "daylight_hours": None,
        "soil_moisture": round(nasa_data["GWETROOT"], 2),
